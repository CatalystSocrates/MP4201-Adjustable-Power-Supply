...
#include <string.h>
#include "screen.h"
uint8_t uart1_recv_frame[MAX_FRAME_LEN];
#if 1
//fputc
...
uint8_t uart1_ring_buf[BUFFER_MAX_LEN];  // 串口1环形缓冲区
...
// -------------------------- 环形缓冲区 --------------------------
//因为解析内容比较简单，而且串口数据发送量较少且相对集中，故直接在此处理数据
void UART_RingBuffer_Write(uint8_t *ring_buf, uint8_t data)
{
